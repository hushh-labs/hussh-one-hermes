...
| Date | Commit | What shipped |
|------|--------|---------------|
| 2026-07-29 | Unreleased | **Passkey-assisted trusted-device enrollment.** Browser approval can reuse an RP-compatible One passkey to unwrap and hash-validate the existing vault key, seal it to an ephemeral Hermes key, and deliver ciphertext exactly once through the existing PKCE exchange. Missing, canceled, stale, or incompatible passkeys fall back to the native masked passphrase ceremony; hosted MCP remains unchanged. |
