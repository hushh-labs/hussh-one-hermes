...
| 2026-08-28 | `81375163` | **Restart without cutting anyone off mid-answer.** Quiesce, drain against the per-session lease registry, publish a readable phase for the app and Puppy One, then hand off to the existing SIGUSR1 path. An abandoned drain names what it would interrupt and does not restart unless forced. |
| 2026-08-28 | `7d2f2923` | **Agent sync against a fleet manifest.** Reuses `distribution.yaml` as the agent manifest and the pod's replace-never-patch semantics. Closes the provenance gap by capturing the resolved ref and a content digest before the swap, refuses user-owned paths at plan time, and never swaps a payload that did not validate. |
