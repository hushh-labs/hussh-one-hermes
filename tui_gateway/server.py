...
def _hussh_one_surface_toolsets(platform: str) -> set[str]:
    """Hussh One native-workstation toolsets, exposed only to trusted surfaces.

    Native PKM tools remain independently gated on an enrolled local
    envelope. Desktop and the loopback browser dashboard (``HERMES_TUI_DASHBOARD``)
    are trusted workstation surfaces; standalone TUI and connected messaging
    channels remain excluded. Off ``_HERMES_CORE_TOOLS`` for the same reason
    as ``_gui_surface_toolsets`` above — every other platform would carry the
    schema for nothing.

    Desktop unconditionally gets ``project`` alongside ``hussh_one`` (even on
    an explicit HERMES_TUI_TOOLSETS pin, unlike the generic
    ``_gui_surface_toolsets`` fold) — the desktop shell is always a local
    workstation surface with project-tool authority.
    """
    if platform == "desktop":
        return {"hussh_one", "project"}
    if is_truthy_value(os.environ.get("HERMES_TUI_DASHBOARD")):
        return {"hussh_one"}
    return set()


def _strip_child_only_toolsets(names) -> set[str]:
    """Drop the child-only Source Library toolset from a parent tool surface.
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    if not explicit:
        try:
            ...
            if selection is not None:
                # Fold in the client-surface and Hussh One toolsets here too:
                # the focus-mode coding posture returns before the fallback
                # path that normally adds them — without this the desktop
    ...
    if explicit and validate_toolset is not None:
        ...
        if not unresolved:
            ...
            # outright, so unlike the posture/config fallback paths below,
    ...
    try:
        ...
        if not enabled:
            return None
        # The Desktop Project/client-surface tools and Hussh One connector are
        # deliberately outside _HERMES_CORE_TOOLS (every other platform would
        # carry their schema for nothing), so the platform recovery above —
        # which keys off hermes-cli's tool universe — can't surface them. This
        # resolver runs ONLY in the desktop/TUI gateway, so folding them in
