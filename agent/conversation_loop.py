...
def run_conversation(
    ...
    # hussh 🤫 One: Fail fast for Vertex Claude runtimes that look configured
    # but cannot run a Hermes-shaped request. This protects long-lived TUI
    # sessions that were switched before the current access preflight existed.
    # (Layered on top of upstream's turn_context refactor — runs once per turn
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # hussh 🤫 One: Vertex Claude can 404 a model for one location while it
        # is reachable from another. Track attempted locations so the in-loop
        # recovery (try_recover_vertex_claude_location) doesn't retry the same
        # one. Kept as a bare local (not in TurnRetryState) since it's a
        # Hussh One customization layered on top of upstream's retry refactor.
