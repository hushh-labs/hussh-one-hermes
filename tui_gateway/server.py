    ...
    def _build() -> None:
        ...
        try:
            ...
            try:
                ...
                #
                # hussh 🤫 One: when there's no explicit resume id, fall back to
                # session_id=key so _make_agent restores this session's last-used
                # model from sessions.model (state.db). Without it the restore
                # guard (`...and session_id`) is False and a resumed session
                # silently reverts to the config default model — e.g. a Claude
                # Opus session coming back as Gemini after a browser refresh.
                # (#hussh-one session-model resume)
                kw = {"session_db": session_db}
                resume_sid = current.get("resume_session_id")
                kw["session_id"] = resume_sid or key
...
def _make_agent(
    ...
    try:
        ...
        # 🤫 Hussh One: if initializing with model_override failed (e.g. stored
        # model's provider is not configured), fall back to the default/startup
        # runtime instead of failing the entire resume/load.
        if isinstance(model_override, dict) and model_override.get("model"):
            logger.warning(
...
def _(rid, params: dict) -> dict:
    ...
    if (t := current_transport()) is not None:
        session["transport"] = t
    with session["history_lock"]:
        # 🤫 Hussh One: intercept a clear natural-language model switch
        # ("switch to opus", "use gemini") and apply it as a /model command
        # instead of running it as an agent turn.
