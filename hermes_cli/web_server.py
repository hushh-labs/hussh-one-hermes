...
def _render_active_theme_bootstrap_css() -> str:
    ...
    try:
        ...
        if any(b["name"] == active for b in _BUILTIN_DASHBOARD_THEMES):
            return ""
        for theme in [*_discover_repo_themes(), *_discover_user_themes()]:
...
def _discover_repo_themes() -> list:
    """Scan distribution-shipped dashboard themes."""
    ...
    for theme_file in sorted(themes_dir.glob("*.yaml")):
        try:
            data = yaml.safe_load(theme_file.read_text(encoding="utf-8"))
        except Exception:
            continue
        normalised = _normalise_theme_definition(data)
        if normalised is not None:
