...
def test_desktop_explicit_pin_still_strips_source_library_leaf(monkeypatch):
    """An explicit HERMES_TUI_TOOLSETS pin wins outright (matches
    tests/tui_gateway/test_gui_surface_toolsets.py::test_explicit_env_pin_still_wins)
    — it does not gain project/hussh_one just because the session is desktop.
    Only the private Steward launch may attach the source leaf toolset, so
    that one strip still applies even under an explicit pin.
    """
