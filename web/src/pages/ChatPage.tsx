...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    ...
    // hussh 🤫 One: PING heartbeat keeps NAT/proxy/idle timers warm and lets
    // the browser notice a half-open socket sooner than the default ~minute TCP
    // timeout. The server consumes `\x1b[PING]` in pty_ws and never forwards it
    // to the PTY child (same local-consume path as RESIZE).
    ...
    return () => {
      ...
      // Detach onclose first so the teardown close doesn't re-enter reconnect.
      const sock = wsRef.current;
      if (sock) {
        sock.onclose = null;
        sock.onerror = null;
