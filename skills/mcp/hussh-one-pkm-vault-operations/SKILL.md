...
### Model-Safe Status Inspection

Use `/hussh-one status` for connection, enrollment, lock, and replica-cursor
status. Do not inspect identity, envelope, lock-state, or replica files with
generic file or terminal tools. Those files are a custody boundary, not model
context.
...
## Reading PKM Information

Use the native `read_my_pkm` tool only:

1. Call `list_domains` when the domain is unknown.
2. Call `read` with the narrowest useful domain and relative `scope_path`.
3. Treat the returned `value` as an authorized, decrypted, memory-only
   projection for the current response.

The stored representation is always an AES-256-GCM ciphertext envelope. Never
call private helpers such as `_snapshot()` or `_decrypt_domain()`, never read or
decrypt replica files directly, and never use an offline/503 condition to bypass
the native read boundary. A service failure remains a failure until the
authorized read path is available.

Never describe the decrypted projection as “the JSON stored in the vault.”
Never merge plaintext with `[VAULT_*]`, `[VAULT_ENCRYPTED]`, or similar
sentinels. Those strings are invalid PKM values, not encryption.
...
- **Size Limits on Reads:** Native reads enforce maximum leaf (500) and JSON byte (65,536 bytes) limits. If a read is too broad, request narrower sub-paths. Never bypass the limit with private decryption helpers.
...
Use `save_to_pkm` for model-initiated writes. It holds the proposal in memory,
shows a safe mutation summary, requires fresh owner approval, rechecks the
source revision, encrypts locally, and commits through the canonical API. Do
not invoke `PkmClient.commit()` from model-authored terminal code.
...
2. **Attribute Names and Safe Preview:** Show the reviewed field names and only
   the minimum masked values needed for informed approval. Do not call this an
   exact stored JSON representation.
...
The confirmation is a decrypted in-memory preview. After approval, the durable
vault and replica contain ciphertext only.
