...
class CreatedVault:
    """One-time local material for the existing Hussh passphrase vault format."""
...
def _new_recovery_key() -> str:
    raw = os.urandom(16).hex().upper()
    return f"HRK-{raw[:4]}-{raw[4:8]}-{raw[8:12]}-{raw[12:16]}"


def create_vault_with_passphrase(passphrase: str) -> CreatedVault:
    """Create wrappers byte-compatible with One's passphrase vault contract.

    The caller owns the returned recovery key and must disclose it once through
    a protected local surface before this material is persisted remotely.
    """
    if len(passphrase) < 8:
        raise VaultCryptoError("The vault passphrase must be at least 8 characters.")
    vault_key = os.urandom(32)
    salt = os.urandom(16)
    iv = os.urandom(12)
    encrypted_vault_key = AESGCM(derive_passphrase_key(passphrase, salt)).encrypt(
        iv, vault_key, None
    )
    recovery_key = _new_recovery_key()
    recovery_salt = os.urandom(16)
    recovery_iv = os.urandom(12)
    recovery_encrypted_vault_key = AESGCM(
        derive_passphrase_key(recovery_key, recovery_salt)
    ).encrypt(recovery_iv, vault_key, None)
    return CreatedVault(
        vault_key=vault_key,
        vault_key_hash=vault_key_hash(vault_key),
        encrypted_vault_key=_encode_binary(encrypted_vault_key),
        salt=_encode_binary(salt),
        iv=_encode_binary(iv),
        recovery_key=recovery_key,
        recovery_encrypted_vault_key=_encode_binary(recovery_encrypted_vault_key),
        recovery_salt=_encode_binary(recovery_salt),
        recovery_iv=_encode_binary(recovery_iv),
    )
...
def unwrap_recovery_vault_key(
    *, recovery_key: str, encrypted_vault_key: str, salt: str, iv: str
) -> bytes:
    """Validate recovery wrappers generated by Hermes or existing One clients."""
    try:
        key = derive_passphrase_key(recovery_key, decode_binary(salt))
        vault_key = AESGCM(key).decrypt(
            decode_binary(iv), decode_binary(encrypted_vault_key), None
        )
    except Exception as exc:
        raise VaultCryptoError("The recovery key is incorrect.") from exc
    if len(vault_key) != 32:
        raise VaultCryptoError("The recovered vault key has an invalid length.")
    return vault_key
