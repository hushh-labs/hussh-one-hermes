...
| **M — Private Source Library control plane** | Provider files remain blob truth; Source Library PKM exposes zero `attr.*` scopes; SQLite holds only opaque/encrypted mapping state; mutations are revision-pinned and freshly approved; revocation removes the real artifact; the Steward has proposal/query tools only | Source Library focused and policy tests |
