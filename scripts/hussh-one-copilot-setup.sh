...
if [[ "$DRY_RUN" != "1" ]]; then
  QUOTED_HERMES_HOME="$(printf '%q' "$HERMES_HOME")"
...
set -euo pipefail
export HERMES_HOME=$QUOTED_HERMES_HOME
export GOOGLE_APPLICATION_CREDENTIALS="\$HOME/.config/gcloud/application_default_credentials.json"
export LITELLM_MASTER_KEY="$KEY"
export GOOGLE_CLOUD_PROJECT="$PROJECT"
exec "\$HERMES_HOME/litellm-venv/bin/litellm" \\
  --config "\$HERMES_HOME/litellm-proxy-config.yaml" \\
  --port $PROXY_PORT \\
  --host 127.0.0.1
EOF
...
set -euo pipefail
export HERMES_HOME=$QUOTED_HERMES_HOME
export LITELLM_MASTER_KEY="\$(grep -o 'LITELLM_MASTER_KEY="[^"]*"' "\$HERMES_HOME/scripts/start_litellm_proxy.sh" | cut -d'"' -f2)"
...
exec "\$HERMES_HOME/litellm-venv/bin/python" "\$HERMES_HOME/scripts/litellm_auth_shim.py"
EOF
