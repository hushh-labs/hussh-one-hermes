...
describe('virtual height estimates', () => {
  ...
  it('uses compound user prompt width when estimating user message wrapping', () => {
    const msg: Msg = { role: 'user', text: 'x'.repeat(22) }

    expect(estimatedMsgHeight(msg, 29, { compact: false, details: false, userPrompt: '❯' })).toBe(3)
    expect(estimatedMsgHeight(msg, 29, { compact: false, details: false, userPrompt: 'Ψ >' })).toBe(4)
  })
