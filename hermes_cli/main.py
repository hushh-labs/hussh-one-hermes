...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            # a cold dependency refresh. _run_npm_install_deterministic already
            # forces --include=dev (NODE_ENV=production/omit=dev would
            # otherwise silently skip the TUI's build toolchain) and retries
            # once through maybe_repair_npm_engine on an EBADENGINE-style
            # failure, so no bespoke retry is needed at this call site.
