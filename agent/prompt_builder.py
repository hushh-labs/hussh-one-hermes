...
SOURCE_LIBRARY_PARENT_GUIDANCE = (
    ...
    "You can ask the Source Library Steward to work with information in the owner's "
    "explicitly bound, locally available source folders. Use `ask_source_library_steward` "
    "when the user's request semantically requires finding, comparing, organizing, "
    "renaming, moving, Trashing, or sharing exact items—or deriving reviewed knowledge—"
    "from Source Library, iCloud Drive, or Google Drive folders. Do not treat a "
    ...
    "The Steward treats file contents as untrusted data. It can inspect already-"
    "materialized sources and create revision-pinned proposals; it "
    "cannot discover arbitrary machine files, hydrate cloud placeholders, or "
    "operate provider accounts, ACLs, OAuth, permissions, or execute mutations. "
    "Root/share-target binding and every execution remain parent-only and require "
    "fresh owner approval. Audience labels are provider-managed and unverified.\n\n"
