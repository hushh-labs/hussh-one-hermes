    ...
    for i, t in enumerate(task_list):
        ...
        try:
            child = _build_child_preserving_parent_tools(
                ...
                # Normal model calls inherit the parent's toolsets; there is
                # no model-facing toolsets arg, so the model itself can never
                # choose or narrow them. Trusted in-process specialist
                # launchers may still narrow a leaf to an exact parent-owned
                # toolset through the non-schema internal argument.
