...
2. **Layered recovery:** launchd owns process restart, the deterministic self-healing doctor checks service ownership every 15 minutes, and the separate LM Studio watchdog checks model inventory every 30 minutes without inference. Only the doctor can issue a bounded service kickstart; the LM Studio check is alert-only and does not restart services or wake the agent.
