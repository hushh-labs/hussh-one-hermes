...
def test_spawn_detached_gateway_starts_retry_watchdog(monkeypatch, tmp_path):
    """_spawn_detached_gateway launches a self-restarting watchdog, not a
    one-shot child: it must run the embedded retry-loop script (so the
    gateway comes back after an abrupt non-zero exit) and record the
    watchdog's own PID so `hermes gateway stop` can find and kill it."""
    calls = []
    child_cmd = [
        "/usr/bin/python3",
        "-m",
    ...
    assert len(calls) == 1
    cmd, kwargs = calls[0]
    # The watchdog is spawned as an embedded Python script (not the child
    # command directly, and not upstream's `hermes_cli.stderr_timestamp`
    # wrapper) so it can retry the child on crash.
    assert cmd[0] == sys.executable
    assert cmd[1] == "-c"
    script = cmd[2]
    assert repr(child_cmd) in script
    assert "while True" in script
    assert kwargs["stdin"] is gateway.subprocess.DEVNULL
    assert kwargs["stdout"].name == str(tmp_path / "logs" / "gateway.log")
    assert kwargs["stderr"].name == str(tmp_path / "logs" / "gateway.error.log")

    watchdog_path = tmp_path / gateway._DETACHED_WATCHDOG_PID_FILE
    assert watchdog_path.read_text(encoding="utf-8") == "4242"
