...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    try:
        ...
        # Capture the message now — `e` is implicitly deleted when this
        # except block exits, so the closure below must not reference it.
        _node_error = str(e)

        def _node_unavailable(args):
            print(f"hermes meet node: module unavailable ({_node_error})")
            return 1
