...
class CLIAgentSetupMixin:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            # hussh 🤫 One: restore the session's last-used model/provider on
            # resume BEFORE the agent is built, so a session last running Claude
            # (re-pinned to GCP Vertex ADC) doesn't silently revert to the
            # config default. Gated on empty history above; the preload path
            # does the equivalent restore itself. (#hussh-one session-model resume)
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        # hussh 🤫 One: this preload path populates conversation_history and
        # thereby skips the equivalent restore inside _init_agent (gated on
        # empty history). Restore the session's last-used model/provider here
        # too, or dashboard resume reverts Claude→Gemini. (#hussh-one resume)
        self._restore_session_model(session_meta)
